<template>
	<div class="bg-orange-700/50 w-[300px] h-[90vh] fixed top-[10vh] pl-5">
		<div class="overflow-y-auto h-[90vh] pr-5">
			...
			<div class="form-item">
				...
				<div v-if="searchByZipcode">
					<OfferInput inputId="zipcode" inputType="number" v-model="zipcode" />
				</div>
			</div>
			...
			<div class="form-item">
				...
				<div v-if="searchByCity" class="relative">
					<OfferInput inputId="city" inputType="text" v-model="city" />
					<font-awesome-icon
				...
				<div v-if="searchByAge">
					<ul class="px-10">
						...
						<li
							...
							@click="
								() => {
									emit('filterByAge', '6-12 mois', 6, 12),
										(searchByAge = false);
								}
							"
						>
	...
	if (route.query.categoryId) {
		axios.get(`${import.meta.env.VITE_URL_BACK}/races`).then((res) => {
			...
			racesListFitered.value = res.data.filter(
				(race: IRaceRes) => race.id_category === Number(route.query.categoryId)
			);
		});
	} else {
		axios.get(`${import.meta.env.VITE_URL_BACK}/races`).then((res) => {
